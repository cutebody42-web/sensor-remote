...
| M3 | Windows capture, codecs, GPU render, input | Real interactive Windows session on supported matrix | IN PROGRESS: attended DXGI/H.264/native viewport/SendInput path implemented; unlocked validation pending |
...
attended chat/files and an implemented attended remote desktop path. Direct
TCP is not M1/M2 completion, and the locked test desktop prevents the final
interactive capture/input gate and any production release claim.
