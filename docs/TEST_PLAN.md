...
[VERIFICATION.md](VERIFICATION.md) records the executed and pending release checks:
...
H.264 encode/decode, media fragment validation, full-duplex transport and
remote input permission gates;
...
Use two independently profiled app instances and then two physical PCs. Start
one attended View session and one attended Control session; verify the remote
frame, monitor selection, pointer, key, text, wheel and ReleaseAll behavior.
...
128 kbps through 100+ Mbps; unlocked capture/input, codec and multi-monitor
tests; UAC/login/lock/reboot;
