...
| Relay | Authenticated preconfigured pair forwarding | relay/desktop | Core + Windows | IN PROGRESS | Real relayed pinned handshake and data; GUI route selection and `sensor-relay.exe` build | No public room allocator, multi-tenant service, relay E2E GUI run, metrics or quotas |
| Internet transport | Signed registration, ephemeral presence, token pairing, heartbeat and opaque WSS relay | render/rendezvous/desktop | Windows + Railway | TESTED | Public Railway WSS chat/file/denial test, `/health`, automatic listener retry | Finite trial allowance, in-memory directory, no direct P2P/ICE/STUN/TURN, no multi-region failover |
| NAT/failover/reconnect | Internet waiting retry and listener restart | render/transport/rendezvous | Windows + Railway | IN PROGRESS | Bounded WSS connect retry, heartbeat and post-session listener restart | Active-session resume, direct-first orchestration, ICE/STUN/TURN and regional failover |
...
| Unattended/login/UAC/CAD | Verified-peer auto-accept while GUI remains open | desktop | Windows attended | IN PROGRESS | Explicit local checkbox and pinned-peer-only auto-accept path | No installed service, password/MFA, startup, secure desktop, UAC or reboot support |
...
| Portable deployment | GUI, CLI and provisioned relay Windows binaries | desktop/client/relay | Windows | IN PROGRESS | Local release builds; resource/icon manifest; package script includes relay | Unsigned; no certified Windows support matrix |
