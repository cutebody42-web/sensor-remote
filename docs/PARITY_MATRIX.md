...
| Render Internet transport | Signed registration, ephemeral presence, token pairing, heartbeat and opaque WSS relay | render/rendezvous/desktop | Windows + Render | TESTED | Native two-client integration test and `/health`; automatic listener retry | Free cold starts, in-memory directory, no direct P2P/ICE/STUN/TURN, no multi-region failover |
| NAT/failover/reconnect | Render waiting retry and listener restart | render/transport/rendezvous | Windows + Render | IN PROGRESS | Bounded WSS connect retry, heartbeat and post-session listener restart | Active-session resume, direct-first orchestration, ICE/STUN/TURN and regional failover |
